...
class Bot {
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const zwspPos = Math.floor(nickname.length/2) + 1;
      const nicknameFixed = nickname.substring(0, zwspPos) + '\u200b' + nickname.substring(zwspPos);
      let text = this.parseText(message);
      let displayUsername = nicknameFixed;
      if (this.ircNickColor) {
        const colorIndex = (nickname.charCodeAt(0) + nickname.length) % NICK_COLORS.length;
        displayUsername = irc.colors.wrap(NICK_COLORS[colorIndex], nicknameFixed);
      }
