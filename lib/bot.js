...
class Bot {
  constructor(options) {
    ...
    /* Load blacklist - list of regexps of irc nicks that should be ignored by the relay */
    this.ircBlacklist = [];
    var ircBlacklistStrings = options.ircNickBlacklist || [];
    ircBlacklistStrings.forEach(ircBlacklistString => {
      this.ircBlacklist.push(new RegExp(ircBlacklistString, ""));
    });
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const zwspPos = Math.floor(username.length/2) + 1;
      const usernameFixed = username.substring(0, zwspPos) + '\u200b' + username.substring(zwspPos);
      let text = this.parseText(message);
      let displayUsername = usernameFixed;
      if (this.ircNickColor) {
        const colorIndex = (username.charCodeAt(0) + username.length) % NICK_COLORS.length;
        displayUsername = irc.colors.wrap(NICK_COLORS[colorIndex], usernameFixed);
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      for (let index = 0; index < this.ircBlacklist.length; ++index) {
        let blackListItem = this.ircBlacklist[index];
        if(blackListItem.test(author)) {
          logger.debug('Ignoring message from IRC user', author, 'in channel', channel);
          return;
        }
      }
